  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden">
      <Helmet>
        ...
        <meta name="description" content="Create an account using OTP or Password" />
      </Helmet>

      {/* Decorative background effects */}
      <div className="absolute top-0 left-0 w-full h-full -z-10 bg-[#010409]" />
      <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-purple-600/10 rounded-full blur-[120px] -z-10 animate-pulse" />
      <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-blue-600/10 rounded-full blur-[120px] -z-10 animate-pulse" style={{ animationDelay: '2s' }} />
      ...
      >
        <div className="glass-card rounded-[2.5rem] p-10 md:p-12 relative overflow-hidden premium-glow glow-purple border-white/[0.05]">
          ...
          <div className="text-center mb-10">
            ...
            >
              ...
              <p className="text-white/40 text-[10px] font-black tracking-[0.2em] uppercase">
                {signupMethod === 'otp'
                  ? (step === 1 ? "Initialize your financial core." : "Decrypting access token Sent To Inbox.")
                  : "Establish account via secure credentials."}
              </p>
            </motion.div>
          </div>
          ...
          <div className="flex p-1.5 bg-white/[0.03] border border-white/[0.05] rounded-[1.2rem] mb-8 relative">
            <button
              onClick={() => { setSignupMethod('otp'); setStep(1); }}
              className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all relative z-10 ${signupMethod === 'otp' ? 'text-white' : 'text-white/30 hover:text-white/50'}`}
            >
              OTP Network
            </button>
            <button
              onClick={() => { setSignupMethod('password'); setStep(1); }}
              className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all relative z-10 ${signupMethod === 'password' ? 'text-white' : 'text-white/30 hover:text-white/50'}`}
            >
              Manual ID
            </button>
            <motion.div
              layoutId="signup-method"
              className="absolute inset-1.5 w-[calc(50%-6px)] bg-purple-600/20 border border-purple-500/30 rounded-xl"
              animate={{ x: signupMethod === 'otp' ? 0 : '100%' }}
          ...
          <AnimatePresence mode="wait">
            {signupMethod === 'otp' ? (
              step === 1 ? (
                ...
                >
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        <User className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-white/10 group-focus-within:text-purple-500 transition-colors" />
                        <input
                          id="name"
                          type="text"
                          name="name"
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        <Mail className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-white/10 group-focus-within:text-purple-500 transition-colors" />
                        <input
                          id="email"
                          type="email"
                          name="email"
                  ...
                  <Button
                    type="submit"
                    disabled={otpLoading}
                    className="w-full bg-purple-600 hover:bg-purple-500 text-white py-8 rounded-[1.5rem] font-black text-[11px] tracking-[0.2em] shadow-2xl shadow-purple-600/20 transition-all transform active:scale-[0.98] group mt-4"
                  >
                    {otpLoading ? (
                ...
                >
                  <div className="space-y-3">
                    ...
                    <div className="relative group">
                      <Lock className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-white/10 group-focus-within:text-purple-500 transition-colors" />
                      <input
                        id="otp"
                        type="text"
                        name="otp"
                  ...
                  <Button
                    ...
                    className="w-full bg-purple-600 hover:bg-purple-500 text-white py-8 rounded-[1.5rem] font-black text-[11px] tracking-[0.2em] shadow-2xl shadow-purple-600/20 transition-all transform active:scale-[0.98]"
                  >
                    {authLoading ? (
                      <RefreshCw className="w-5 h-5 animate-spin" />
                    ) : (
              ...
              >
                <div className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="pm-name" className="text-white/30 text-[10px] font-black uppercase tracking-[0.3em] ml-2">Identify As</Label>
                    <div className="relative group">
                      <User className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-white/10 group-focus-within:text-purple-500 transition-colors" />
                      <input
                        id="pm-name"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="pm-email" className="text-white/30 text-[10px] font-black uppercase tracking-[0.3em] ml-2">Network Hub</Label>
                    <div className="relative group">
                      <Mail className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-white/10 group-focus-within:text-purple-500 transition-colors" />
                      <input
                        id="pm-email"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="pm-password" className="text-white/30 text-[10px] font-black uppercase tracking-[0.3em] ml-2">Access Key</Label>
                    <div className="relative group">
                      <Lock className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-white/10 group-focus-within:text-purple-500 transition-colors" />
                      <input
                        id="pm-password"
                ...
                <Button
                  ...
                  className="w-full bg-purple-600 hover:bg-purple-500 text-white py-8 rounded-[1.5rem] font-black text-[11px] tracking-[0.2em] shadow-2xl shadow-purple-600/20 transition-all transform active:scale-[0.98] mt-4"
                >
                  {authLoading ? (
                    <RefreshCw className="w-5 h-5 animate-spin" />
                  ) : (
                    <span className="flex items-center justify-center gap-3">
                      CREATE IDENTITY <ArrowRight className="w-5 h-5" />
                    </span>
                  )}
                </Button>
              </motion.form>
          ...
          <div className="mt-10 text-center pt-8 border-t border-white/[0.05]">
            <p className="text-white/20 text-[10px] font-black uppercase tracking-widest leading-loose">
              Already Registered?{' '}
              <Link to="/otp-login" className="text-white hover:text-purple-400 transition-all ml-2 underline underline-offset-[12px] decoration-white/10 hover:decoration-purple-400/50 block mt-2">
                Log In Tunnel
              </Link>
            </p>
          </div>
